...
const { getBaiduAccessToken, getDataFromTableImage   } = require('../common/js/baidu.js');
  ...
  compressImage: function (way) {
    chooseImage(way).then(url => {
      ...
      getBaiduAccessToken().then( token => {
        return getDataFromTableImage(token, {
          image: data,
        }) 
      }).then(res => {
        wx.hideLoading();
        console.log(JSON.parse(res.data.result.result_data))
